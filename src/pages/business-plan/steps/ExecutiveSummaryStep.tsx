...
import { Form, Input, Button, Typography, Space, Alert, Spin } from 'antd';
import { BusinessPlanData } from '../../../interfaces/businessPlan.interface';
import { executiveSummaryService, ExecutiveSummaryResponse } from '../../../services/executiveSummary.service';
...
const ExecutiveSummaryStep: React.FC<ExecutiveSummaryStepProps> = ({ data, onNext }) => {
  ...
  const [apiResponse, setApiResponse] = useState<ExecutiveSummaryResponse | null>(null);
      ...
      >
        ...
        >
          <TextArea
            ...
            autoSize={{ minRows: 2, maxRows: 4 }} onBlur={onFieldBlur}
          />
        </Form.Item>
        ...
        <Form.Item
          ...
          help="A brief overview of your business plan (1-2 paragraphs). The summary will be generated automatically when you complete the vision and mission."
        >
          <div style={{ position: 'relative' }}>
            <TextArea
              placeholder={loading ? "Generating summary..." : "Summarize your business plan..."}
              autoSize={{ minRows: 4, maxRows: 8 }}
              disabled={loading}
              value={form.getFieldValue('executiveSummary')}
              onChange={e => form.setFieldsValue({ executiveSummary: e.target.value })}
            />
            {loading && (
